...
const getFalseyPropertyNames_1 = __importDefault(require("../utils/getFalseyPropertyNames"));
const AuthServices_1 = __importDefault(require("./AuthServices"));
class AuthController {
    constructor() {
        this._REFRESH_TOKEN_MAX_AGE = 1000 * 60 * 60 * 24 * 30; // 30 days //
        this._ACCESS_TOKEN_MAX_AGE = 1000 * 60 * 60; // 1 hour //
        this._service = new AuthServices_1.default();
    ...
    async signup(req, res) {
        try {
            ...
            if (!user || !user.email || !user.password || !user.name || !user.name) {
                ...
                const falseyFieldNames = (0, getFalseyPropertyNames_1.default)(user);
    ...
    ;
    async refreshAuthCookies(req, res) {
        try {
    ...
    ;
    async clearAuthCookies(req, res) {
        try {
