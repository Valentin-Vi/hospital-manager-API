...
function Inventario() {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await servApi('/inv/selectAll', { method: 'POST' });
  ...
  async function bulkDelete(e) {
    await servApi('/inv/deletemany', {
      method: 'POST',
      body: JSON.stringify({ items: selectedRows.selectedRows }),
    });
    setReload(prev => !prev);
  };
  
  function bulkConsume() {
    servApi('/inv/subtractmany', {
      method: 'POST',
      body: JSON.stringify({ items: selectedRows.selectedRows})
    })
    .then(res => {
  ...
  return (
    <div className='flex flex-col'>
      <label className='text-xl'>Inventario</label>
      <div className='flex flex-row justify-around p-2 m-3'>  
        ...
        <button onClick={bulkDelete} className='rounded pt-1 pb-1 pr-2 pl-2 bg-red-200 shadow-md shadow-red-200 active:bg-red-400 active:shadow-red-400  hover:bg-red-300'>
          Borrar
        </button>
